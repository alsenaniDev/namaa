    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.background }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={[styles.container, { paddingTop: topPad, paddingBottom: bottomPad }]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.infoBox, { flexDirection: dir.row, backgroundColor: colors.primary + '0D', borderColor: colors.primary + '25' }]}>
          ...
          <Text style={[styles.infoText, { textAlign: dir.textAlign, color: colors.foreground }]}>{t.setup.sampleHint}</Text>
        </View>

        <Button title={loading ? t.setup.loadingBtn : t.setup.withSampleBtn} onPress={() => handleFinish(true)} fullWidth loading={loading} style={{ marginBottom: 10 }} />
        <Button title={t.setup.emptyBtn} onPress={() => handleFinish(false)} variant="outline" fullWidth disabled={loading} />
      </ScrollView>
